 ...
 * 
 * @note: Based upon the Symfonist fork of Pear Cache Lite with added support
 * for file directory mode and removed Pear Error dependency:
 * 
...
class Cache_Lite
{
    ...
    */
     ...
     * File Dir mode
     * 
     * Use id to determine the directory struct
     */
    var $_fileDirMode = true;
    ...
    */
    function get($id, $group = 'default', $doNotTestCacheValidity = false)
    {
        ...
        if ($this->_caching) {
            ...
            //var_dump(date('r', $this->_refreshTime));var_dump($this->_file);var_dump(date('r', @filemtime($this->_file)));exit;
            //according to docs this is unecessary
            //clearstatcache();
            ...
            if (($doNotTestCacheValidity) || (is_null($this->_refreshTime))) {
                ...
                $valid = true;
                if (file_exists($this->_file) && $valid = (($this->_fileTime = @filemtime($this->_file)) > $this->_refreshTime)) {
                    $data = $this->_read();
                }
                elseif(!$valid){//clean garbage on the fly
    ...
    * @access public
    */
    function save($data, $id = NULL, $group = 'default', $index = 'index.html')
    {
        if ($this->_caching) {
            if ($this->_automaticSerialization) {
                $data = serialize($data);
            ...
            if (isset($id)) {
                $this->_setFileName($id, $group, $index, true);//will write the dirs
            }
            if ($this->_memoryCaching) {
                $this->_memoryCacheAdd($data);
                if ($this->_onlyMemoryCaching) {
            ...
            if ($this->_writeControl) {
                ...
                //if (is_bool($res)) {
                    ...
                    throw new RuntimeException('Corrupt file data!');
    ...
    * @access public
    */
    function remove($id, $group = 'default', $index = 'index.html', $checkbeforeunlink = false)
    {
        $this->_setFileName($id, $group, $index);
        if ($this->_memoryCaching) {
            if (isset($this->_memoryCachingArray[$this->_file])) {
        ...
        $result = $this->_unlink($this->_file);
        //remove the dir if it's empty
        @rmdir(dirname($this->_file));//will throw a notice if the directory is not empty
        return $result;
    }
    ...
    */
    ...
    function has($id, $group = 'default', $doNotTestCacheValidity = false)
    {
        $this->_id = $id;
        $this->_group = $group;
        //$data = false;
        if ($this->_caching) {
            ...
            if (($doNotTestCacheValidity) || (is_null($this->_refreshTime))) {
                ...
                $valid = true;
                if ((file_exists($this->_file)) && $valid = (@filemtime($this->_file) > $this->_refreshTime)) {
                    //$data = $this->_read();
                    return true;
                }
                elseif(!$valid){//clean garbage on the fly
    ...
    */
    ...
    {
        $this->_lifeTime = (int)$newLifeTime;
        $this->_setRefreshTime();
    }
    ...
    * @access public
    * @note Removed Pear Error class dependencies, triggers notice
    */
    ...
    */
    ...
    {
        if (0 === $this->_lifeTime) {//0 is infinite
    ...
    */
    function _cleanDir($dir, $group = false, $mode = 'ingroup')     
    {
        ...
        elseif ($this->_fileNameProtection) {
        ...
        while (($file = readdir($dh)) !== false) {
            if (($file != '.') && ($file != '..')) {
                if ($this->_fileDirMode || substr($file, 0, 6)=='cache_') {//add filemode, not sure why legacy uses substr
                    $file2 = $dir . $file;
                    if (is_file($file2)) {
                        switch (substr($mode, 0, 9)) {
                            case 'old':
                                ...
                                break;
                            case 'notingrou'://not applicable in file dir mode
                            ...
                            case 'callback_':
                                ...
                                break;
                            case 'ingroup':
                            default://$motif will be empty string in file dir mode
                                if ('' === $motif || strpos($file2, $motif) !== false) {
                                    $result = ($result and ($this->_unlink($file2)));
                                }
                                break;
                        }
                    }
                    if ((is_dir($file2)) && ($this->_fileDirMode || $this->_hashedDirectoryLevel>0)) {
    ...
    * @access private
    */
    function _setFileName($id, $group, $index = 'index.html', $write = false)
    {
        $this->writeHeaders = false;
        if(!$this->_fileDirMode){
            $suffix = 'cache_' . ($this->_fileNameProtection ? md5($group) . '_' . md5($id) : $group . '_' . $id);
        }
        else{
            if(false !== strpos($id, DIRECTORY_SEPARATOR)){
                $fileDirs = array_filter(explode(DIRECTORY_SEPARATOR, $id), array($this, 'filterPath'));
                //no way determine if uri mapping is file or dir, so if it isn't a file name it's a dir
                $suffix = 0 === count($fileDirs) || false === strpos(end($fileDirs), '.') ? $index : array_pop($fileDirs);
            ...
            if($write && !is_dir($root)){
                mkdir($root, 0777);
            }
            $root .= DIRECTORY_SEPARATOR;
            if(!empty($fileDirs)){
                $root .= implode(DIRECTORY_SEPARATOR, $fileDirs);
                if($write && !is_dir($root)){
        ...
        if ($this->_fileNameProtection && $this->_hashedDirectoryLevel>0) {
            $hash = md5($suffix);
    ...
    */
    function _write($data)
    {
        ...
        /*if($this->writeHeaders){
            $fp = @fopen(substr_replace($this->_file , 'h', -1), "wb");
            if ($fp) {
                if ($this->_fileLocking) @flock($fp, LOCK_EX);
                @fwrite($fp, serialize(headers_list()));
                if ($this->_fileLocking) @flock($fp, LOCK_UN);
                @fclose($fp);
            }  
        }*/
        ...
        //returns false instead of using Pear error object
    ...
    */
    function _writeAndControl($data)
    {
        ...
        if (!$result) {
            return $result; # We return the PEAR_Error object @depricated return false
        }
        $dataRead = $this->_read();
        # We return the PEAR_Error object @depricated returns false
        return false === $dataRead ? $dataRead : $dataRead == $data;
    }
